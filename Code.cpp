	...
	void Collision(int dir){
		for (int i = rect.top / 32; i < (rect.top + rect.height) / 32; i++){
			for (int j = rect.left / 32; j < (rect.left + rect.width) / 32; j++){
				...
				if (TileMap[i][j] == 'B' || TileMap[i][j] == 'w' || TileMap[i][j] == 'q' || TileMap[i][j] == 'e'|| TileMap[i][j] =='r'){
...
void menu(RenderWindow & window) {
	...
	menuTexture3.loadFromFile("image/exit.png");
	...
	while (isMenu)
	{
		...
		menuNum = 0;
		window.clear(Color(72, 201,239)); //////фон ебанутого меню
		if (IntRect(100, 30, 200, 100).contains(Mouse::getPosition(window))) { menu1.setColor(Color::Color(188,182,199)) , menuNum = 1; }
		if (IntRect(100, 450, 200, 100).contains(Mouse::getPosition(window))) { menu3.setColor(Color::Color(188,182,199)), menuNum = 3; }
...
int main()
{
	...
	RenderWindow window(VideoMode(1000, 600), "Test game beta0.02");
	...
	while (window.isOpen()){
		...
		for (int i = 0; i < H; i++){
			for (int j = 0; j < W; j++){
				if (TileMap[i][j] == 'B') { tile.setTextureRect(IntRect(470, 550, 32, 32)); tile.setPosition(j * 32 - offsetX, i * 32 - offsetY); }
				...
				if (TileMap[i][j] == 'w') { tile.setTextureRect(IntRect(405, 132, 30, 30)); tile.setPosition(j * 32 - offsetX, i * 32 - offsetY); }
